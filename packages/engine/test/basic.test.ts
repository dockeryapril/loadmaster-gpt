...
import { computeNegotiation, DEFAULT_NEGOTIATION_SETTINGS, LoadData, UserSettings } from '../src/index.ts';

test('computeNegotiation calculates rates with baseline RPM', () => {
  const load: LoadData = { miles: 100, rate: 1000 };
...
test('computeNegotiation returns null when miles or rate missing', () => {
  const load: LoadData = { miles: 0, rate: 0 };
  const userSettings: UserSettings = {
    rpmThresholds: { excellent: 2, good: 1.5, fair: 1 }
  };
