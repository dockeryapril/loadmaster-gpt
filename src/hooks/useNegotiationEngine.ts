...
export function useNegotiationEngine({ load, laneBaselineRpm }: UseNegotiationEngineProps) {
  ...
  const result = useMemo(() => {
    ...
    const fields = {
      ...
      weekend: load.accessorials?.weekend,
      afterHours: load.accessorials?.afterHours,
      inside: load.accessorials?.inside,
      residential: load.accessorials?.residential,
      liftgate: load.accessorials?.liftgate,
      palletJack: load.accessorials?.palletJack,
      detentionHours: load.accessorials?.detentionHours,
